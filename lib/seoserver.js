...
getContent = function(url, callback) {
    var content = '',
        phantom = require('child_process').spawn('phantomjs', ['--ignore-ssl-errors=true', __dirname + '/phantom-server.js', url]);
    ...
    phantom.on('exit', function(code) {
        if (code !== 0) {
            ...
            var clean = cheerio.load(content).find('[data-requirecontext]').remove();
            callback(clean.html());
        }
    });
};
